...
export type ArtworkShape = 'rounded' | 'circle'
...
export type SpectrumMode = 'off' | 'economy' | 'smooth'
export type SpectrumOrigin = 'bottom' | 'center'
...
export interface SettingsPayload {
  ...
  readonly rotateCircularArtwork: boolean
  readonly showProgress: boolean
  readonly spectrumMode: SpectrumMode
  readonly spectrumOrigin: SpectrumOrigin
...
/** 从设置载荷中读取实时频谱档位；旧设置缺少字段时默认关闭。 */
export function readSpectrumMode(settings: SettingsPayload | undefined): SpectrumMode {
  const mode = settings?.spectrumMode
  return mode === 'economy' || mode === 'smooth' ? mode : 'off'
}

/** 从设置载荷中读取频谱增长方向；旧设置默认从容器底部向上增长。 */
export function readSpectrumOrigin(settings: SettingsPayload | undefined): SpectrumOrigin {
  return settings?.spectrumOrigin === 'center' ? 'center' : 'bottom'
}
...
/** 从设置载荷中读取封面形状，旧设置默认保持原有圆角方形。 */
export function readArtworkShape(settings: SettingsPayload | undefined): ArtworkShape {
  return settings?.artworkShape === 'circle' ? 'circle' : 'rounded'
}

/** 从设置载荷中读取圆形封面旋转开关，旧设置默认关闭。 */
export function readRotateCircularArtwork(settings: SettingsPayload | undefined): boolean {
  return settings?.rotateCircularArtwork === true
}
