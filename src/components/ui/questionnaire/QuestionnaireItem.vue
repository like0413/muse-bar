...
function getAnswerByShortcut(shortcut: string) {
  if (shortcutByChoiceValue.value) {
    ...
    return (
      answers.value.find((answer) => answer.type === 'choice' && answer.value === choiceValue) ??
      null
    )
  }
...
watch(
  () => props.multiple,
  (multiple, wasMultiple) => {
    if (!wasMultiple || multiple) {
      return
    }

    const selectedAnswer = answers.value.find((answer) =>
      selectedAnswerIds.value.includes(answer.id),
    )
...
<template>
  <fieldset
    ...
    :inert="!active"
    tabindex="-1"
    :class="cn('flex min-w-0 flex-col gap-5 border-0 p-0 outline-none', props.class)"
  >
