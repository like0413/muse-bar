...
连续终止 Explorer、切换自动隐藏以及真实多屏 DPI 变化会影响桌面环境，不在普通测试中自动执行；
发布前按原验证矩阵人工故障注入。纯状态规则已有自动测试覆盖恢复请求首次触发、冷却合并和冷却后重试。
...
新增测试覆盖媒体运行时退避、空缓存重试、支持播放器选择、损坏封面和 Unicode 文本上限。真实播放器
退出、拒绝控制和 WinRT 服务故障需要外部进程配合，发布前使用四个目标播放器执行人工矩阵。
...
新增后台线程测试覆盖正常 join 和超时放弃等待。Vue 两个页面继续由 Pinia store 唯一持有监听器，
页面卸载统一调用 `stop()`；异步晚到的监听器会立即自行注销，没有新增组件级全局订阅。
...
- `vp test --passWithNoTests`：通过；当前前端尚无测试文件。
- `cargo test --locked`：27 项 Rust 测试全部通过，其中本章新增 12 项。
