...
2. 当前版本只实现 Child，不加入 Owner 宿主和自动回退逻辑。
3. Owner 兼容模式保留为后续计划，用于未来 Windows 版本、第三方任务栏或挂载失败环境。
4. Explorer 重启恢复仍由唯一的 Rust 生命周期管理器负责，前端窗口不创建全局监听器。

## 后续验证

阶段 5 需要继续验证正式 Child 封装不会改变当前输入行为。阶段 8 加入真实媒体控制后，
补验按钮连续点击、拖动 seek 和长时间悬停。阶段 12 再覆盖 Windows 版本、DPI、任务栏高度、
多屏、锁屏恢复、第三方任务栏修改器和不同类型的全屏应用。
