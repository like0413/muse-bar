...
impl AppState {
    ...
    pub fn report_bar_content_width(
        ...
        if !additional_width.is_finite()
            || additional_width < 0.0
            || additional_width > f64::from(u32::MAX)
        {
            return Err("Bar 上报的附加区域宽度无效".to_owned());
        }

        let settings = self.settings()?;
        let maximum_width = settings.max_width;
        let rounded_width = natural_width.ceil() as u32;
        let rounded_additional_width = additional_width.ceil() as u32;
        let (target_width, mode) = match available_area_width {
            Some(width) => (width.max(1), BarWidthMode::AvailableArea),
            None => (
                rounded_width
                    .min(maximum_width)
                    .saturating_add(rounded_additional_width),
                BarWidthMode::Content,
            ),
        };
