...
use windows::{
    ...
    Win32::{
        ...
        Media::Audio::{
            eRender, AudioSessionStateActive, Endpoints::IAudioMeterInformation,
...
enum VolumeRequest {
    ...
    ResolveCaptureProcess {
        identity: MediaVolumeIdentity,
        response: SyncSender<Result<Option<u32>, ApplicationVolumeError>>,
    },
    Shutdown,
}
...
impl ApplicationVolumeManager {
    ...
    /// 返回最适合作为 WASAPI 进程回环目标的活动音频进程。
    pub(crate) fn resolve_capture_process(
        &self,
        identity: MediaVolumeIdentity,
    ) -> Result<Option<u32>, ApplicationVolumeError> {
        self.request(|response| VolumeRequest::ResolveCaptureProcess { identity, response })
    }
...
fn run_volume_worker(receiver: Receiver<VolumeRequest>) {
    ...
    while let Ok(request) = receiver.recv() {
        match request {
            ...
            VolumeRequest::ResolveCaptureProcess { identity, response } => {
                let _ = response.send(resolve_capture_process(&identity));
            }
...
/// 保留当前媒体应用的会话峰值接口，供受保护音频无法回环采样时驱动可视化。
pub(crate) struct ApplicationPeakMeter {
    meters: Vec<IAudioMeterInformation>,
}

impl ApplicationPeakMeter {
    pub(crate) fn open(identity: &MediaVolumeIdentity) -> Result<Option<Self>, String> {
        let sessions = collect_matching_sessions(identity).map_err(|error| error.to_string())?;
        let Some(highest_match_score) = sessions.iter().map(|session| session.match_score).max()
        else {
            return Ok(None);
        };
        let has_active_session = sessions
            .iter()
            .any(|session| session.match_score == highest_match_score && session.active);
        let meters: Vec<_> = sessions
            .into_iter()
            .filter(|session| {
                session.match_score == highest_match_score
                    && (!has_active_session || session.active)
            })
            .filter_map(|session| session.meter)
            .collect();
        Ok((!meters.is_empty()).then_some(Self { meters }))
    }

    pub(crate) fn peak(&self) -> f32 {
        self.meters
            .iter()
            .filter_map(|meter| unsafe { meter.GetPeakValue() }.ok())
            .fold(0.0_f32, f32::max)
            .clamp(0.0, 1.0)
    }
}

fn resolve_capture_process(
    identity: &MediaVolumeIdentity,
) -> Result<Option<u32>, ApplicationVolumeError> {
    let sessions = collect_matching_sessions(identity)?;
    let highest_match_score = sessions.iter().map(|session| session.match_score).max();
    Ok(highest_match_score.and_then(|score| {
        sessions
            .iter()
            .filter(|session| session.match_score == score)
            .max_by_key(|session| session.active)
    ...
    for device_index in 0..device_count {
        ...
        for session_index in 0..session_count {
            ...
            let meter = control
                ...
                .inspect_err(|error| log::debug!("音频会话不提供峰值接口：{error}"))
                .ok();
